...
lookup_id(CustId) ->
  usr_db2:lookup_id(CustId).

%% Service API

lookup_msisdn(PhoneNo) ->
  usr_db2:lookup_msisdn(PhoneNo).

service_flag(PhoneNo, Service) ->
  case usr_db2:lookup_msisdn(PhoneNo) of
...
init(FileName) ->
  usr_db2:create_tables(FileName),
  usr_db2:restore_backup(),
  {ok, null}.

terminate(_Reason, _LoopData) ->
  usr_db2:close_tables().

handle_cast(stop, LoopData) ->
  {stop, normal, LoopData}.

handle_call({add_usr, PhoneNo, CustId, Plan}, _From, LoopData) ->
  Reply = usr_db2:add_usr(#usr{msisdn=PhoneNo,
                              id=CustId,
                              plan=Plan}),
  {reply, Reply, LoopData};

handle_call({delete_usr, CustId}, _From, LoopData) ->
  Reply = usr_db2:delete_usr(CustId),
  {reply, Reply, LoopData};

handle_call({set_service, CustId, Service, Flag}, _From, LoopData) ->
  Reply = case usr_db2:lookup_id(CustId) of
    {ok, Usr} ->
      ...
      usr_db2:update_usr(Usr#usr{services=NewServices});
    {error, instance} ->
      {error, instance}
  end,
  {reply, Reply, LoopData};
...
handle_call({set_status, CustId, Status}, _From, LoopData) ->
  Reply = case usr_db2:lookup_id(CustId) of
    {ok, Usr} ->
      usr_db2:update_usr(Usr#usr{status=Status});
    {error, instance} ->
      {error, instance}
  end,
  {reply, Reply, LoopData};
...
handle_call(delete_disabled, _From, LoopData) ->
  {reply, usr_db2:delete_disabled(), LoopData}.
